    ...
    TransformVariableSignRestrictions(List<double> objFunc, List<List<double>> constraints, List<VariableSignType> varSigns)
                ...
                if (isConsoleOutput)
                {
                    ...
                    foreach (var transformation in variableTransformations)
                    {
                        switch (transformation.Type)
                        {
                            ...
                            case "integer":
                            case "binary":
                                Logger.WriteLine($"x{transformation.OriginalIndex + 1} ({transformation.Type}) -> x'{transformation.TransformedIndex + 1}");
                                Logger.WriteLine($"x{transformation.OriginalIndex + 1} ({transformation.Type}) -> x'{transformation.TransformedIndex + 1}");
                                break;
                            case "negative":
                                Logger.WriteLine($"x{transformation.OriginalIndex + 1} (negative) -> -x'{transformation.TransformedIndex + 1}");
                                break;
                            case "unrestricted":
                                Logger.WriteLine($"x{transformation.OriginalIndex + 1} (unrestricted) -> x'{transformation.TransformedIndex + 1} - x'{transformation.TransformedIndexNeg + 1}");
                                break;
                        }
                    }
                    Logger.WriteLine();
            ...
            public void DoPreliminaries(List<double> objFunc, List<List<double>> constraints, bool isMin, List<VariableSignType> varSigns)
            {
                ...
                if (isConsoleOutput)
                {
                    ...
                    Logger.WriteLine("Transformed Objective Function:");
                    Logger.WriteLine($"Maximize {string.Join(" + ", transformedObjFunc.Select((c, i) => $"{c}x'{i + 1}"))}");
                    Logger.WriteLine();

                    Logger.WriteLine("Transformed Constraints:");
                    for (int i = 0; i < transformedConstraints.Count; i++)
                    {
                        ...
                        Logger.WriteLine($"  {constraintStr} {operator_str} {transformedConstraints[i][transformedConstraints[i].Count - 2]}");
                    }
                    Logger.WriteLine();
                }
                ...
                if (isConsoleOutput)
                {
                    Logger.WriteLine($"Basic variable spots: [{string.Join(", ", basicVarSpots)}]");
                }
                ...
                for (int i = 0; i < firstTab.Count; i++)
                {
                    ...
                    {
                        Logger.Write($"{firstTab[i][j],-15:F2} ");
                    }
                    Logger.WriteLine();
                }
                ...
                for (int i = 0; i < revisedTab.Count; i++)
                {
                    for (int j = 0; j < revisedTab[i].Count; j++)
                    {
                        Logger.Write($"{revisedTab[i][j],-15:F2} ");
                    }
                    Logger.WriteLine();
                }
                ...
                if (isConsoleOutput)
                {
                    ...
                    {
                        Logger.WriteLine($"x'{i + 1} = {transformedSolution[i]:F4}");
                    }

                    Logger.WriteLine("\nOriginal Solution (after back-transformation):");
                    for (int i = 0; i < originalSolution.Count; i++)
                    {
                        Logger.WriteLine($"x{i + 1} = {originalSolution[i]:F4}");
                    }
            ...
            private void VerifySignRestrictions(List<double> solution, List<VariableSignType> varSigns)
            {
                ...
                for (int i = 0; i < Math.Min(solution.Count, varSigns.Count); i++)
                {
                    ...
                    Logger.WriteLine($"x{i + 1} = {value:F4} ({signType}) - {status}");
                }
