...
namespace lpr381Project
{
    ...
    public class BranchAndBoundKnapsack
    {
        ...
        public List<KnapsackItem> DisplayRatioTest()
        {
            ...
            {
                Logger.WriteLine($"{item.Name}    {item.Value}/{item.Weight} = {item.Ratio:F3}    {rankMap[item.Index]}");
            }
            Logger.WriteLine();
                                       ...
                                       string subProblemNumber, bool isRoot = false)
        {
            ...
            {
                Logger.WriteLine($"Sub-Problem {subProblemNumber}");
            }
            else
            {
                Logger.WriteLine("Sub-Problem");
            }
            ...
            foreach (int itemIndex in sortedFixedKeys)
            {
                ...
                if (value == 1)
                {
                    Logger.WriteLine($"* {item.Name} = {value}    {remainingCapacity}-{item.Weight}={remainingCapacity - item.Weight}");
                    remainingCapacity -= item.Weight;
                ...
                {
                    Logger.WriteLine($"* {item.Name} = {value}    {remainingCapacity}-0={remainingCapacity}");
                    fixedItems.Add(itemIndex);
                }
            }
            ...
            foreach (var item in sortedItems)
            {
                ...
                if (remainingCapacity >= item.Weight)
                {
                    Logger.WriteLine($"{item.Name} = 1    {remainingCapacity}-{item.Weight}={remainingCapacity - item.Weight}");
                    remainingCapacity -= item.Weight;
                    totalValue += item.Value;
                }
                else if (remainingCapacity > 0)
                {
                    ...
                    Logger.WriteLine($"{item.Name} = {fractionalDisplay}    {remainingCapacity}-{item.Weight}");
        ...
        public void DisplayIntegerModel()
        {
            ...
            Logger.WriteLine($"s.t {weightStr} ≤ {capacity}");
        ...
        private void FinalizeCandidate(KnapsackNode node, List<int> candidateCounter)
        {
            ...
            if (selectedItems.Count > 0)
            {
                ...
                Logger.WriteLine($"z = {string.Join(" + ", valueTerms)} = {(int)node.Profit}");
            }
            else
            {
            ...
            Logger.WriteLine($"Candidate {candidateLetter}");
        ...
        public class KnapSack
        {
            ...
            public void RunBranchAndBoundKnapSack(List<double> objFuncPassed, List<List<double>> constraintsPassed, bool isMin, List<VariableSignType> varSigns = null)
            {
                ...
                Logger.WriteLine($"Maximum value: {bestValue}");
                Logger.WriteLine("Solution vector:");

                for (int i = 0; i < bestSolution.Count; i++)
                {
                    Logger.WriteLine($"x{i + 1} = {bestSolution[i]}");
                }
                ...
                Logger.WriteLine($"\nVerification:");
                Logger.WriteLine($"Total weight: {totalWeight} (≤ {capacity})");
                Logger.WriteLine($"Total value: {totalValue}");
            }
        }
    }
}